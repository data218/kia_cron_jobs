...
function getData() {
  ...
  else if(curView==='high') d = d.filter(r=>(Number(r.netodpremiuma)||0)>=50000);
  return d;
}
...
function render() {
  ...
  // KPI
  const tp = d.reduce((s,r)=>s+(Number(r.netodpremiuma)||0),0);
...
function drawTrend(d) {
  const m = {}; d.forEach(r=>{const k=r.create_date?.substring(0,7); if(!k)return; if(!m[k])m[k]={p:0,pr:0}; m[k].p++; m[k].pr+=Number(r.netodpremiuma)||0;});
...
function drawComparison(d) {
  ...
  d.forEach(r=>{
    ...
    m[mo][y].p++; m[mo][y].pr += Number(r.netodpremiuma)||0;
  });
...
function drawCalculations(d) {
  ...
  const rows = [
    ...
    ['KPI: Gross Premium', 'sum of netodpremiuma for filtered rows', '₹'+(tp/100000).toFixed(1)+'L', '₹'+(tp/100000).toFixed(1)+'L / ₹'+(tp/10000000).toFixed(1)+'Cr'],
    ...
    ['Filter: View (High)', 'netodpremiuma >= ₹50,000', curView==='high' ? 'Active' : '-', 'getData() → netodpremiuma>=50000'],
    ['', '', '', ''],
...
function getTableData(d) {
  ...
  const sorted=[...d].sort((a,b)=>(Number(b.netodpremiuma)||0)-(Number(a.netodpremiuma)||0));
  return sorted.slice(0,15).map(r=>`<tr><td style="font-family:monospace;font-size:10px">${r.policyno||'-'}</td><td>${r.customer_name||'-'}</td><td>${r.model||'-'}</td><td>${(r.insurancecompany||'-').substring(0,22)}</td><td><strong>₹${fmt(Number(r.netodpremiuma)||0)}</strong></td><td>${r.create_date||'-'}</td></tr>`);
}

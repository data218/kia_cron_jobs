#!/usr/bin/env node
// Akhnoor (N5701) Remaining Sales Report Fetcher (March 2021 -> June 2026)
// Login: N570100 / Hyundai@01 | Dealer: N5701 (Preserves active portal dealer)
...
import { getThirtyDayChunks, parseIsoLocalDate } from '../src/utils/date-range.js';
...
const START_DATE = getArg('start', '2021-03-01');
...
async function exportAndSaveChunk(reportContext, chunk, chunkDir) {
  ...
  try {
    ...
    if (stat.isFile()) {
      console.log(`[SKIP] Chunk ${chunk.startIso} -> ${chunk.endIso} already saved.`);
  ...
  if (!exportResult.exportedFiles || exportResult.exportedFiles.length === 0) {
    console.log(`[INFO] No records found for ${chunk.startPortal} -> ${chunk.endPortal} (0 rows).`);
  ...
  await saveReportSheetToSupabase({
...
async function main() {
  console.log('===============================================================');
  console.log('  HYUNDAI SALES REPORT - AKHNOOR (N5701) REMAINING RUNNER');
  console.log('===============================================================');
  ...
  const downloadDir = path.resolve(`./downloads/hmil-akhnoor-sales`);
  ...
  try {
    ...
    console.log('\n===============================================================');
    console.log(`🎉 COMPLETED ALL CHUNKS FOR AKHNOOR (${DEALER_CODE})`);
    console.log(`   Total Rows Inserted: ${totalSaved}`);
    console.log('===============================================================');

  } catch (err) {
    console.error(`\n❌ Error during execution: ${err.message}`);
    const errorShot = path.join(downloadDir, `akhnoor-sales-error-${Date.now()}.png`);
